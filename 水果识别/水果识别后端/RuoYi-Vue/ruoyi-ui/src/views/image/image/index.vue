<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="变化" prop="mark">
        <el-input
    ...
    <el-table v-loading="loading" :data="imageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="水果id" align="center" prop="id" />
      <el-table-column label="水果名称" align="center" prop="name" />
      <el-table-column label="水果图片路径" align="center" prop="image" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.image" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="变化" align="center" prop="mark" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="水果图片路径">
          <image-upload v-model="form.image"/>
        </el-form-item>
        <el-form-item label="变化" prop="mark">
          <el-input v-model="form.mark" placeholder="请输入变化" />
        </el-form-item>
      </el-form>
...
<script>
import { listImage, getImage, delImage, addImage, updateImage } from "@/api/image/image";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        mark: [
          { required: true, message: "变化不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除image编号为"' + ids + '"的数据项？').then(function() {
        return delImage(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('image/image/export', {
        ...this.queryParams
